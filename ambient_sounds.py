...
class AmbientSounds():
    def __init__(self):
        ...
        self.sound_set_1 = [SfPlayer(self.sounds["birds"], loop=True, mul=self.fader1)]
        self.sound_set_2 = [SfPlayer(self.sounds["dining_hall"], loop=True, mul=self.fader2)]
    ...
    def change_sound(self, new_sound):
        ...
        if self.current_sound_set == 1:
            self.sound_set_2[0].setPath(self.sounds[new_sound])
            self.fader1.stop()
            self.sound_set_2[0].out()
            self.fader2.play()
            self.current_sound = 2
            # print("if")
        else:
            self.sound_set_1[0].setPath(self.sounds[new_sound])
            self.fader2.stop()
            self.sound_set_1[0].play()
            self.fader1.play()
            self.current_sound = 1
            # print("else")
            
    def start_first_sound(self, first_sound):
        self.sound_set_1[0].setPath(self.sounds[first_sound])
        self.fader1.play()